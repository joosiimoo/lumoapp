...
`SaleSession.status` MUST be `open` after a successful start and MUST remain `open` after a successful add-item. Transitions to confirmed, pending_information, or voided MUST NOT be implemented in this change.
