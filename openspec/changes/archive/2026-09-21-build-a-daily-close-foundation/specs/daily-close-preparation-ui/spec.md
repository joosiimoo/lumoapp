...
`GenerativeUIRegistry` MUST register component `daily_close_preparation` version `1`. `GenerativeUIComposer` MUST emit this contract for a completed `closing.prepare@1` read and for a completed `closing.submit_cash_count@1` write, including the not-started and not-counted states. `actions` MUST be empty. Money MUST be decimal strings plus the business currency. `text` MUST equal `fallback_text`. The payload MUST NOT include a close-confirmation action, a reopen action, an approval, an exception list, a product breakdown, a chart series, history, or accounting fields. PRD §10.5 names `closing_ready_card` and `cash_difference_card` MUST remain unregistered.
...
- **WHEN** `expected_cash` is `22.50`, the current count is `20.00`, and the business date is `2026-09-21`
- **THEN** the agent response `ui` MUST include exactly one `daily_close_preparation` version `1` whose `cash_status` is `short`, `cash_difference.amount` is `-2.50`, and `fallback_text` contains `Faltante`

#### Scenario: Composer emits the not-counted card
- **WHEN** the day has cash sales of `22.50` and no `CashCount`
- **THEN** `counted_cash`, `cash_difference`, `counted_at`, and `cash_count_id` MUST be null, `cash_status` MUST be `not_counted`, and `fallback_text` MUST contain `Falta contar efectivo` and MUST NOT contain a difference
...
The capture turn and the preparation card MUST appear in the existing Inicio conversation stream as assistant artifacts: unbubbled, Lumo-mark gutter, warm canvas, max-width 420px, existing spacing and radius tokens. Inicio MUST keep the same `conversation_id` when sending cash-count and preparation phrases. Navigation, the four-tab shell, the composer, and the theme MUST be unchanged. Hoy, Memoria, and Negocio MUST remain placeholders. The Design System "Preparar el cierre del día" action card, the Hoy hero metric card, the hourly chart, and a closing-flow screen MUST NOT be built in this change, and no new dark theme, accent hue, motion, or skeleton MUST be introduced.
