...
Minimum OperationalDay for Build A: one business date per business, status `open` or `closed`, confirmed sales attached at `sale.commit@1`, a server-side daily summary that stays a live aggregate after close, and optional append-only cash counts that belong to that day. Only `closing.confirm@1` moves a day from `open` to `closed`.
