# factual-memory-read Specification

## Purpose

Factual memory is a closed, typed read over facts Lumo has already stored. The result is not persisted, and it is not a narrative, a score, or a search index.
...
When a query resolves one `OperationalDay`, `source_coverage` MUST be the recorded-operations declaration for that day, including empty domains and sources when no coverage row exists. The declaration MUST keep `basis=recorded_operations`, domains and sources sorted, `merchant_source_declaration` null, and `limitation_code=only_lumo_registered_operations`. It MUST NOT contain a percentage, a confidence, a health score, or a complete flag. `recent_cash_differences` MUST leave `source_coverage` null.
...
Executing any factual memory query MUST NOT insert or update a business event, a coverage row, an `OperationalDay`, a sale, a payment, a `CashCount`, a `ClosingSnapshot`, an OutcomeRun, a WorkItem, an audit row, an outbox row, or an idempotency row. A repeated read with unchanged stored facts MUST return the same factual result. The tenant MUST come from trusted context. The query MUST NOT accept a caller-supplied `business_id` and MUST NOT use `BYPASSRLS`.
