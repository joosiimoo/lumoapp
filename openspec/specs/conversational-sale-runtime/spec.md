...
Agent message API, registered catalog/sale/day/closing-preparation tools including `sale.totalize@1`, `sale.commit@1`, `operational_day.summary@1`, `closing.submit_cash_count@1`, and `closing.prepare@1`, policies, scripted local interpreter, and application workflows that resolve then commit start/reuse + add-item, totalize to `ready_to_charge`, record payment to `confirmed`, record a cash count, or read close preparation.
