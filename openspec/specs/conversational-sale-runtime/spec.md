...
#### Scenario: Cuánto vendí hoy is the sales summary
- **WHEN** the actor posts `¿Cuánto vendí hoy?`
- **THEN** the decision MUST select `memory.business_facts@1` with `query_type` `sales_summary` for the business-local today
