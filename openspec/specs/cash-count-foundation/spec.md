...
Append-only `CashCount` for daily close preparation: merchant-provided counted cash, one current row per `OperationalDay`, tenant-safe supersede chain, and `closing.submit_cash_count@1`. This is not Daily Close confirmation.
