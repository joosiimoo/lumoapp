...
- **THEN** `status` MUST be `closed` and `gross_sales_total.amount` MUST be the live aggregate `56.50`, and the payload MUST NOT include `expected_cash` or `closing_snapshot_id`
...
- **WHEN** the payload has `status` `closed`
- **THEN** the card MUST show `Cerrado` and MUST NOT compute that state from the totals
