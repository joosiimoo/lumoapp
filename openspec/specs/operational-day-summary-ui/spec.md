...
`business_date` MUST be an ISO calendar date. `status` MUST be `open` or `closed` when `operational_day_id` is present and MUST be null when it is not. `sale_count` of `1` MUST use `1 venta` in `fallback_text`; every other count MUST use `N ventas`. Method labels in that sentence MUST be `Efectivo`, `Tarjeta`, and `Transferencia`. Amounts in the sentence MUST be the server decimal strings. `data.gross_sales_total.amount` MUST equal the sum of the three method amounts. PRD name `daily_summary_card` MUST remain unregistered.

#### Scenario: Composer emits a one-sale summary
- **WHEN** today's confirmed sales are one cash sale of `56.50` MXN on an open business date `2026-09-21`
- **THEN** the agent response `ui` MUST include exactly one `operational_day_summary` version `1` whose `sale_count` is `1`, `gross_sales_total.amount` is `56.50`, `cash_total.amount` is `56.50`, the other method amounts are `0.00`, `status` is `open`, `actions` is empty, and `fallback_text` contains `1 venta`

#### Scenario: Composer emits zeros without a day id
- **WHEN** the summary is today's date and no OperationalDay exists
- **THEN** `operational_day_id` and `status` MUST be null, `sale_count` MUST be `0`, every amount MUST be `0.00`, `actions` MUST be empty, and `fallback_text` MUST contain `0 ventas`

#### Scenario: Closed day keeps live totals
- **WHEN** today's OperationalDay is `closed` and its confirmed sales total `56.50`
- **THEN** `status` MUST be `closed`, `actions` MUST be empty, `gross_sales_total.amount` MUST be the live aggregate `56.50`, and the payload MUST NOT include `expected_cash` or `closing_snapshot_id`
...
Flutter `GenerativeUIRenderer` MUST register `operational_day_summary` version `1`. It MUST render a compact Inicio card using the existing Lumo mark gutter, soft `LumoCard`, and the same chip and type treatment as the other conversation cards: the server business date, sale count, gross total, and cash, card, and transfer amounts. When `status` is `closed`, it MUST show the server label `Cerrado` and MUST NOT infer that label by comparing amounts. Flutter MAY format the ISO `business_date` and decimal strings for display. It MUST NOT derive the calendar day from the device timezone, MUST NOT add method totals, and MUST NOT replace `gross_sales_total`. It MUST NOT show a close button, expected cash, a difference, exceptions, a chart, or history. Unknown versions MUST show `fallback_text` and MUST NOT run actions. When `text` equals `fallback_text`, the visible stream MUST NOT also show that sentence as a prose block.
...
#### Scenario: Closed label comes from status
- **WHEN** `status` is `closed`
- **THEN** the card MUST show `Cerrado` and MUST NOT show a close or reopen control
...
The summary MUST appear in the existing Inicio conversation stream as an assistant artifact: unbubbled, Lumo-mark gutter, warm canvas, max-width 420px. Navigation, the four-tab shell, the composer, and the theme MUST be unchanged. The summary card MUST NOT add a chart, a weekly comparison, a dashboard layout, or a download action. `actions` MUST remain empty. Hoy MAY show the sales export panel from `sales-export-ui` and the next-best-action block from `next-best-action-ui`. Hoy MUST NOT be required to stay an empty placeholder. Hoy MUST NOT become a closing-flow screen, a chart, a task list, or a reporting dashboard because of the summary card or the next-action block.

#### Scenario: Ventas de hoy on Inicio
- **WHEN** the signed-in Carrota user submits `ventas de hoy`
- **THEN** the stream MUST show the user bubble and a Lumo-mark `operational_day_summary@1` card whose count and totals match the server payload, and the Hoy tab MUST NOT become a closing-flow screen
