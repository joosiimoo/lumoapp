...
- **WHEN** the GET runs twice for an open day that already has one WorkItem
- **THEN** both responses MUST return that same `work_item_id`, and WorkItem and audit row counts MUST be unchanged
...
- **THEN** the decision MUST be `deny` under `NBA-001` and no WorkItem MUST be written from those arguments
...
- **THEN** both responses MUST return that `work_item_id`, and WorkItem, audit, and idempotency row counts MUST be unchanged
